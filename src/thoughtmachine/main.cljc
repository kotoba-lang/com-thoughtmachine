(ns thoughtmachine.main
  "Kotodama WASM entrypoint for the Thoughtmachine clean-room actor (L4) — Clojure port.
  ...
  facts (`thoughtmachine.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "thoughtmachine")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "thoughtm_acc"
    :fields [:holderId :type :balance :currency :status]           :required [:type :balance]
    :coerce {:balance :float}                    :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "thoughtm_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"thoughtmachine.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "thoughtmachine-compat" :tier tier :entities entities} 200])
...
(defn start! [] :thoughtmachine-compat/ready)
